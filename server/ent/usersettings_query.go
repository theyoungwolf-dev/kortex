...
func (usq *UserSettingsQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (usq *UserSettingsQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (usq *UserSettingsQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (usq *UserSettingsQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (usq *UserSettingsQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (usq *UserSettingsQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := usq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
//		GroupBy(usersettings.FieldCreateTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.UserSettings.Query().
//		Select(usersettings.FieldCreateTime).
//		Scan(ctx, &v)
...
func (usq *UserSettingsQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*UserSettings, init func(*UserSettings), assign func(*UserSettings, *User)) error {
	...
	nodeids := make(map[uuid.UUID][]*UserSettings)
	for i := range nodes {
		if nodes[i].user_settings == nil {
			continue
		}
		fk := *nodes[i].user_settings
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_settings" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (usq *UserSettingsQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(usersettings.Table, usersettings.Columns, sqlgraph.NewFieldSpec(usersettings.FieldID, field.TypeUUID))
	_spec.From = usq.sql
