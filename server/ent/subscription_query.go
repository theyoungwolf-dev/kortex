...
type SubscriptionQuery struct {
	...
	withUser            *UserQuery
	withCheckoutSession *CheckoutSessionQuery
	withFKs             bool
...
// QueryCheckoutSession chains the current query on the "checkout_session" edge.
func (sq *SubscriptionQuery) QueryCheckoutSession() *CheckoutSessionQuery {
	query := (&CheckoutSessionClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SubscriptionQuery) Clone() *SubscriptionQuery {
	...
	return &SubscriptionQuery{
		...
		withCheckoutSession: sq.withCheckoutSession.Clone(),
...
// WithCheckoutSession tells the query-builder to eager-load the nodes that are connected to
// the "checkout_session" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *SubscriptionQuery) WithCheckoutSession(opts ...func(*CheckoutSessionQuery)) *SubscriptionQuery {
	query := (&CheckoutSessionClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withCheckoutSession = query
	return sq
}
...
func (sq *SubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subscription, error) {
	var (
		...
		loadedTypes = [2]bool{
			sq.withUser != nil,
			sq.withCheckoutSession != nil,
		}
	)
	if sq.withUser != nil || sq.withCheckoutSession != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withCheckoutSession; query != nil {
		if err := sq.loadCheckoutSession(ctx, query, nodes, nil,
			func(n *Subscription, e *CheckoutSession) { n.Edges.CheckoutSession = e }); err != nil {
			return nil, err
		}
	}
...
func (sq *SubscriptionQuery) loadCheckoutSession(ctx context.Context, query *CheckoutSessionQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *CheckoutSession)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Subscription)
	for i := range nodes {
		if nodes[i].checkout_session_subscription == nil {
			continue
		}
		fk := *nodes[i].checkout_session_subscription
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(checkoutsession.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "checkout_session_subscription" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
