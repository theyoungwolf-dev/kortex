...
export default async function OGImage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const logo = (await readFile(join(process.cwd(), "public", "logo.png")).then((buff) => Uint8Array.from(buff))).buffer;
    ...
    >
      ...
      >
        <div style={{ color: "#64748B", display: "flex", fontSize: 20 }}>
          ...
          <span>{new Date(blogPost.metadata.publishedAt).toLocaleDateString()}</span> · <span>{minutes} min read</span>
        </div>
        <img src={logo as any} alt="Kortex 1 logo" style={{ width: 96, height: 96, objectFit: "contain" }} />
      </div>
    </div>,
    {
      ...size,
