...
export async function generateMetadata(): Promise<Metadata> {
  return {
    metadataBase: new URL(
      new URL((process.env.COOLIFY_URL ?? process.env.BASE_URL ?? "http://localhost:3001").split(",")[0]).origin,
    ),
    title: "Kortex 1",
    description:
  ...
  return (
    <html lang="en" className="dark">
      ...
      <body className={`antialiased ${inter.className} bg-background`}>
        <Providers session={session} serverUrl={process.env.SERVER_URL!} basePath={process.env.BASE_PATH ?? ""}>
