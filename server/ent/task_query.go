...
func (tq *TaskQuery) QueryCar() *CarQuery {
	query := (&CarClient{config: tq.config}).Query()
...
func (tq *TaskQuery) QueryMods() *ModQuery {
	query := (&ModClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TaskQuery) WithCar(opts ...func(*CarQuery)) *TaskQuery {
	query := (&CarClient{config: tq.config}).Query()
...
func (tq *TaskQuery) WithMods(opts ...func(*ModQuery)) *TaskQuery {
	query := (&ModClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withCar; query != nil {
		if err := tq.loadCar(ctx, query, nodes, nil,
			func(n *Task, e *Car) { n.Edges.Car = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := tq.withMods; query != nil {
		if err := tq.loadMods(ctx, query, nodes,
			func(n *Task) { n.Edges.Mods = []*Mod{} },
			func(n *Task, e *Mod) { n.Edges.Mods = append(n.Edges.Mods, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range tq.withNamedMods {
		if err := tq.loadMods(ctx, query, nodes,
			func(n *Task) { n.appendNamedMods(name) },
			func(n *Task, e *Mod) { n.appendNamedMods(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (tq *TaskQuery) loadCar(ctx context.Context, query *CarQuery, nodes []*Task, init func(*Task), assign func(*Task, *Car)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Task)
	for i := range nodes {
		if nodes[i].car_tasks == nil {
			continue
		}
		fk := *nodes[i].car_tasks
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "car_tasks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TaskQuery) loadSubtasks(ctx context.Context, query *TaskQuery, nodes []*Task, init func(*Task), assign func(*Task, *Task)) error {
	...
	for _, n := range neighbors {
		fk := n.task_subtasks
		if fk == nil {
			return fmt.Errorf(`foreign-key "task_subtasks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "task_subtasks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (tq *TaskQuery) loadMods(ctx context.Context, query *ModQuery, nodes []*Task, init func(*Task), assign func(*Task, *Mod)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(task.ModsTable)
		s.Join(joinT).On(s.C(mod.FieldID), joinT.C(task.ModsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(task.ModsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(task.ModsPrimaryKey[0]))
...
func (tq *TaskQuery) WithNamedSubtasks(name string, opts ...func(*TaskQuery)) *TaskQuery {
	query := (&TaskClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedSubtasks == nil {
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithNamedMods(name string, opts ...func(*ModQuery)) *TaskQuery {
	query := (&ModClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedMods == nil {
		tq.withNamedMods = make(map[string]*ModQuery)
	}
	tq.withNamedMods[name] = query
	return tq
}
